...
// firebase database
import { db } from "../../firebseInit";
import { collection, addDoc, onSnapshot } from "firebase/firestore";
// toast notification
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

// initial State to store data for list of all the users, if a user is  logged, data of loggedIn user
...
export const getInitialUserList = createAsyncThunk(
  ...
  (args, thunkAPI) => {
    ...
    const unsub = onSnapshot(collection(db, "buybusy2"), (snapShot) => {
...
export const createUserThunk = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    ...
    const docRef = await addDoc(collection(db, "buybusy2"), {
      name: data.name,
      email: data.email,
